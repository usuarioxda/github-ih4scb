import { PromptEvaluationResult, PromptDetails } from './types';

export const evaluatePromptScore = (prompt: string): number => {
  const details = {
  ...
  return calculateOverallScore(details);
};
...
const calculateOverallScore = (details: PromptDetails): number => {
